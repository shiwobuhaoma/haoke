package com.xinze.haoke.module.drivers.presenter;
...
import com.xinze.haoke.module.drivers.view.DriverAddActivity;
...
public class MyDriverPresenterImp implements IMyDriverPresenter {
    ...
    @Override
    public void getMyCooperatedDrivers(int pageNum, int pageSize, String driverName) {
        HashMap<String, String> headers = HeaderConfig.getHeaders();
        RetrofitFactory.getInstence().Api().getMyCooperatedDrivers(headers, pageNum, pageSize, driverName).enqueue(new Callback<ReturnResult<List<TruckownerDriverVO>>>() {
